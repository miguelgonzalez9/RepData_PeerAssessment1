...
An additional variable which indicates the day that the observation was taken was added to the dataset.  
Grouped by dat varible and summarized steps variable.
```{r histogram}
...
```{r}
AvgStp <- with(actData, tapply(steps, date, mean))
...
The following for loop loops over each observation in steps and if it's NA replaces it with the mean of all the observation with the same interval.
```{r missing values}
